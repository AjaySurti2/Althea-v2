/*
  # Fix Health Insights Table Schema

  1. Problem
    - health_insights table has incorrect columns (insight_type, insight_text)
    - Should have insights_data (jsonb), tone, language_level
    - Edge function cannot insert data due to schema mismatch

  2. Solution
    - Drop and recreate health_insights table with correct schema
  ...
  3. Columns
    ...
    - insights_data: jsonb NOT NULL (the AI-generated insights)
    - tone: text (conversational, professional, reassuring)
    - language_level: text (simple_terms, educated_patient, medical_professional)
    - metadata: jsonb (additional info like report_count, test_count)
    - report_id: uuid (cached report reference)
    - report_storage_path: text (cached report location)
